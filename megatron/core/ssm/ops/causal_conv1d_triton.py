...
def causal_conv1d_update(
    ...
    """Update causal convolution state and compute outputs with Triton.

    Args:
        x (torch.Tensor): Input tensor with shape ``(batch, sequence, channels)`` or
            ``(batch, channels)``.
        conv_state (torch.Tensor): Persistent convolution state tensor.
        weight (torch.Tensor): Per-channel convolution weights.
        bias (torch.Tensor | None): Optional per-channel bias.
        silu_activation (bool): Whether to apply SiLU to convolution outputs.
        conv_state_indices (torch.Tensor | None): Source state index for each batch row.
        conv_state_write_indices (torch.Tensor | None): Destination state index for each batch row.
        intermediate_conv_states (torch.Tensor | None): Optional per-token state output buffer.
        state_bank_count (int): Number of flattened state banks per logical request slot.

    Returns:
        torch.Tensor: Convolution outputs with the same shape as ``x``.
    """
