...
def selective_state_update(
    ...
    """Update selective-scan state and compute outputs with Triton.

    Args:
        state: ``(batch, dim, dstate)`` or ``(batch, nheads, dim, dstate)`` state tensor.
        x: Input matching a supported state layout, optionally with sequence and head dimensions.
        dt: Time-step tensor matching ``x``.
        A: ``(dim, dstate)`` or ``(nheads, dim, dstate)`` state-transition tensor.
        B: ``(batch, dstate)``, ``(batch, seqlen, dstate)``, ``(batch, ngroups, dstate)``, or
            (batch, seqlen, ngroups, dstate)
        C: Output-projection tensor matching ``B``.
        D: Optional ``(dim,)`` or ``(nheads, dim)`` skip tensor.
        z: Optional gate tensor matching ``x``.
        dt_bias: Optional time-step bias.
        dt_softplus: Whether to apply softplus to ``dt``.
        state_batch_indices: Optional source state index for each batch row.
        state_batch_write_indices: Optional destination state index for each batch row.
        intermediate_ssm_states: Optional per-token state output buffer.
        state_bank_count: Number of flattened state banks per logical request slot.

    Returns:
        Tensor: Selective-scan output matching the original shape of ``x``.
    """
