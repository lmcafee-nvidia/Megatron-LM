...
class TextGenerationController:
    ...
    def _dynamic_step_sample_logits(self):
        ...
        # Async-scheduling primitive: mirror sample[N] into the GPU staging
        # buffer that forward[N+1]'s CUDA graph reads as input_ids. Only
        # safe on pure-decode, non-speculative steps (one token per
        # request matches token_to_input_ids[:active_request_count]).
        # Behavior is byte-identical when async-scheduling is off because
        # transfer_bookkeeping_to_gpu overwrites this slot at the start
        # of step N+1.
    ...
    def _launch_decode_step(self) -> Optional[Dict]:
        ...
        Synchronously enqueues GPU work (forward pass, MTP verify/rewind/serial-MTP
    ...
    async def async_generate_output_tokens_dynamic_batch(
        ...
        # Yield to the asyncio event loop so other CPU work can overlap with
        # the sample-kernel GPU execution. (The async-scheduling engine driver
