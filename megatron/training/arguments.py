...
def _add_inference_args(parser):
    ...
    group.add_argument(
        ...
        'buffers, sized to min(max(0, ceil(max_tokens / block_size) - 1), '
        '2 * max_requests) slots); the scratch is reserved first, so '
        'a smaller max_tokens (or max_requests) shrinks the scratch '
