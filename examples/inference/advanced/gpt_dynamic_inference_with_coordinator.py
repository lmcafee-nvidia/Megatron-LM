...
from examples.inference.advanced.gpt_dynamic_inference import _build_dynamic_engine as build_engine
from examples.inference.advanced.gpt_dynamic_inference import (
    _collect_nested_numeric_pairs as _collect_nested_pairs,
)
from examples.inference.advanced.gpt_dynamic_inference import (
    add_runner_args,
...
def _assert_numeric_pairs(pairs, p95_atol, max_atol):
    """Check cache-off/cache-on log-probability parity within explicit tolerances."""
    ...
    assert p95 <= p95_atol, stats
    assert max_difference <= max_atol, stats
...
def _stress_snapshot(engine, first_group_hashes, previous_hits, previous_skipped):
    ...
    values = [
        ...
        int(engine._prefix_cache_hits > previous_hits),
        int(engine._prefill_tokens_skipped > previous_skipped),
    ...
    if stress_cycles is not None:
        ...
        for cycle_idx in range(stress_cycles):
            if dist.get_rank() == 0:
                ...
                client.pause_engines()
            await engine.wait_until(EngineState.PAUSED)
            state, allocated = _stress_snapshot(
                engine, first_group_hashes, previous_hits, previous_skipped
            )
            previous_hits = engine._prefix_cache_hits
            previous_skipped = engine._prefill_tokens_skipped
            stress_snapshots.append((state, allocated))
    ...
    with torch.inference_mode():
        ...
        if args.prefix_cache_compare:
            ...
            if dist.get_rank() == 0:
                ...
                assert all(s[0][2] == world_size and s[0][3] == world_size for s in snapshots)
                assert all(s[0][5 if ref_zero else 6] == world_size for s in snapshots)
                assert ref_zero or all(s[0][4] == 0 for s in snapshots)
