...
def assert_numeric_pairs(pairs):
    assert pairs, "HTTP responses did not contain any numeric logprobs"
...
def assert_all_ranks_stressed(server_log_dir):
    """Require cache reuse and full-pool pressure on every inference rank."""
    assert server_log_dir, "prefix-cache stress requires per-rank server logs"
    hit_ranks, full_pool_ranks = set(), set()
    for log in Path(server_log_dir).rglob("std*.log"):
        try:
            rank = int(log.parent.name)
        except ValueError:
            continue
        contents = log.read_text()
        if re.search(r"prefix cache \(cumul\): [1-9][0-9]* hits", contents):
            hit_ranks.add(rank)
        utilization = re.findall(r"blocks: occupied ([0-9]+)/([0-9]+), allocatable", contents)
        if any(int(occupied) == int(usable) for occupied, usable in utilization):
            full_pool_ranks.add(rank)

    expected_ranks = set(range(WORLD_SIZE))
    assert hit_ranks == expected_ranks, f"prefix-cache hits reached ranks {hit_ranks}"
    assert (
        full_pool_ranks == expected_ranks
    ), f"cache pressure filled pools on ranks {full_pool_ranks}"


def run_server(args, prefix_cache=False):
    server_log_dir = (
        str(Path(args.server_log_dir) / ("cache-on" if prefix_cache else "cache-off"))
...
def main() -> int:
    ...
    assert args.server_log_dir, "prefix-cache comparison requires --server-log-dir"
    ...
    assert_all_ranks_stressed(Path(args.server_log_dir) / "cache-on")
    print("[smoke] PASS: cache-off/on HTTP outputs match under cache pressure", flush=True)
    return 0
