    ...
    with torch.inference_mode():
        ...
        with real_engine(config, role=role, backend="nixl", weights=weights) as engine:
            ...
            def capture_source(request, prepared):
                ...
                local["source"] = engine.context.memory_buffer[:, :, blocks].cpu().clone()
                local["source_id"] = request.request_id

            def capture_import(pending):
                assert pending.handle.poll()
                local["imported"] = (
                    engine.context.memory_buffer[:, :, pending.local_blocks].cpu().clone()
                )
                local["transfer"] = {
            ...
            with (
                ForwardWitness(engine, request_id=0) as witness,
                ...
                if rank == 0:
                    ...
                    if streaming:
                        ...
                        assert len(partials) >= 2 and all(len(part) >= 2 for part in partials)
                        assert incremental == expected[: len(incremental)]
                    else:
