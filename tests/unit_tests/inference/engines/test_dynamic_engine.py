...
class TestDynamicInferenceEngine(DynamicInferenceEngineTestBase):
    ...
    def test_ep_dummy_forward_participates_in_reuse_agreement_without_pending(
        ...
        """Dummy ranks with no pending forward still join EP async handoff checks."""
        controller = object.__new__(TextGenerationController)
        calls = []
        context = types.SimpleNamespace(
            is_decode_only=lambda: True,
    ...
    def test_ep_dummy_forward_no_graph_async_handoff_observes_idle(self, monkeypatch) -> None:
        """A no-graph dummy step can still clear stale EP global-work consensus."""
        ...
        assert fake.calls == [(0, 1), (0, 0, 0, 0, 0, 0, 0)]
        assert calls == [
            ("init", {"is_dummy_forward": True}),
    ...
    def test_ep_dummy_forward_mtp_no_graph_async_handoff_observes_idle(
        self, monkeypatch
    ) -> None:
        """MTP dummy ranks also join no-graph async handoff checks."""
    ...
    def test_ep_async_launch_before_sampling_sends_skip_without_cuda_graph(self) -> None:
        """Real EP ranks still pair dummy handoff checks when no graph can launch."""
        controller = object.__new__(TextGenerationController)
        dims = InferenceBatchDimensions(token_count=1, prefill_req_count=0, decode_req_count=1)
        calls = []
        ...
        controller.inference_wrapped_model = types.SimpleNamespace(inference_context=context)
        controller.num_speculative_tokens = 0
        controller._async_disable_reason = "not using cuda graph"
        controller._ep_async_collective_enabled = lambda: True
        controller._async_decode_batch_dimensions = lambda active_request_count: dims
        controller._record_async_disable_reason = lambda reason: calls.append(
            ("disabled", reason)
        ...
        assert calls == [
            ...
            ("disabled", "not using cuda graph"),
        ]

    def test_ep_async_launch_before_sampling_cancels_prepared_non_graph_step(self) -> None:
        """A prepared non-graph step must vote skip rather than launch."""
        controller = object.__new__(TextGenerationController)
        dims = InferenceBatchDimensions(token_count=1, prefill_req_count=0, decode_req_count=1)
        calls = []
    ...
    def test_ep_mtp_prepare_after_sampling_sends_skip_without_cuda_graph(self) -> None:
        """MTP real ranks still pair dummy handoff checks when no graph can launch."""
        controller = object.__new__(TextGenerationController)
        dims = InferenceBatchDimensions(token_count=3, prefill_req_count=0, decode_req_count=1)
        calls = []
        ...
        controller._async_scheduling_disabled_reason = (
            lambda allow_mtp=False: "not using cuda graph"
        )
        controller._record_async_eligibility_result = lambda reason: calls.append(
            ("eligible", reason)
        )
        ...
        controller._get_async_forward_graph = lambda: calls.append(("graph",)) or object()

        result = controller._try_prepare_async_decode_after_sampling(
            synchronize_ep_launch=True
        )

        assert not result
        assert calls == [
            ("eligible", "not using cuda graph"),
            (
                "agreement",
                {
                    "local_has_work": True,
