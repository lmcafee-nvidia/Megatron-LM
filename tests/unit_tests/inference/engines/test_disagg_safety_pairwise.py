...
@pytest.mark.parametrize(
    ("model", "backend"),
    [
        pytest.param("gpt", "nixl", id="kv-nixl"),
        pytest.param("hybrid", "nccl", id="ssm-nccl"),
...
def _exercise_owned_source_state(
    mixer, gpt, transport_world, model, backend, stop_keep, *, resuming, cycles
):
    """Exercise source ownership through the requested handoff cycles."""
    ...
    with dtu.real_engine(config, role="prefill" if source else "decode", backend=backend) as engine:
        ...
        for cycle in range(cycles):
            mixer.reset_mock()
            gpt.reset_mock()
            if cycle and (resuming == "both" or source == (resuming == "prefill")):
                engine.suspend()
                engine.resume()
            ...
            if source:
                assert engine._poll_pending_kv_pushes() == (backend == "nccl")
                del buffer, request
