...
class TestDynamicContext:
    ...
    @pytest.mark.internal
    @rounder_override(64)
    def test_prepare_async_decode_next_step_pre_sampling_declines_token_termination(self):
        dynamic_context = self._get_dynamic_context(
            params_dtype=torch.float32,
            num_layers=2,
            kv_channels=8,
        ...
        requests = [
            DynamicInferenceRequest(
                request_id=request_id,
                prompt_tokens=torch.arange(0, 1, device='cpu'),
                sampling_params=SamplingParams(num_tokens_to_generate=31, termination_id=7),
            )
            for request_id in [10, 11, 12]
        ]
        dynamic_context.add_dummy_requests_parallel(requests, count_as_prefill=False)
