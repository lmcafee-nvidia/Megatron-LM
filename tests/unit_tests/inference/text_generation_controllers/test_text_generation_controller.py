...
def _make_async_sched_controller(context=None, model_config=None):
    ...
    context.kv_block_allocator = SimpleNamespace(
        block_routing={}, store_routing_per_block=mock.Mock()
    )
...
@pytest.mark.parametrize(
    "termination_ids, stop_word_finished_ids, next_forward_done_event, expected_wait",
    [
        ([99, 99, 99], set(), "forward", False),
        ([99, 2, 99], set(), "forward", True),
        ([99, 99, 99], {11}, "forward", True),
        ([99, 99, 99], {11}, None, False),
    ],
)
def test_run_async_sched_resolve_waits_only_for_finish_boundary(
    ...
    sample_result = SimpleNamespace(
        sampled_tokens_cpu_view=sample_tokens, accepted_tokens_cpu_view=None, routing_record=None
    )
    result = controller._run_async_sched_resolve(
        sample_result, context.get_active_sequence_lengths() + 1, next_forward_done_event
    )
    ...
    assert result.finished_routing_block_ids == ({11: [41]} if expected_mask[1].item() == 0 else {})
    if next_forward_done_event is None:
        controller._compact_async_sched_forward.assert_not_called()
    else:
...
class TestTextGenerationController(TextGenerationControllerTestBase):
    ...
    def test_dynamic_sampling_keeps_sampled_tokens_buffer_full_capacity(self):
        ...
        every sampling path. Both the non-speculative and async-scheduling paths write
        their `active_request_count` prefix without rebinding the buffer. The buffer must
        retain its full capacity across successive steps regardless of each step's active
        count, so a later step with more active requests than an earlier one still has an
        ...
        sampled = controller._run_async_sched_sample().sampled_tokens_gpu
