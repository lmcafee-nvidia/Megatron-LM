...
def _make_async_sched_context(total_request_count=2, paused_request_count=0):
    ...
    return SimpleNamespace(
        ...
        reserve_async_sched_rewound_kv_blocks=mock.Mock(),
        release_async_sched_rewound_kv_blocks=mock.Mock(),
...
def test_validate_async_sched_support_for_step_accepts_mtp():
    """Async scheduling accepts one configured MTP depth per speculative token."""
    controller = _make_async_sched_controller(_make_async_sched_context(total_request_count=2))
...
def test_validate_async_sched_support_for_step_accepts_log_probs(
    materialize_only_last_token_logits,
):
    context = _make_async_sched_context(total_request_count=2)
    context.config.materialize_only_last_token_logits = materialize_only_last_token_logits
    context.request_metadata["return_log_probs"] = torch.tensor([True, True])
    context.request_metadata["top_n_logprobs"] = torch.tensor([0, 3])
...
def test_async_sched_logits_compaction_moves_complete_mtp_groups():
    ...
    expected_token_rows = torch.tensor([0, 1, 2, 6, 7, 8], dtype=torch.int64)
    assert torch.equal(controller._all_logits_cuda, logits[:, expected_token_rows, :])
    assert torch.equal(
...
def test_async_sched_log_probs_materializes_mixed_top_n():
    context = _make_async_sched_context(total_request_count=3)
    context.request_metadata["return_log_probs"].fill_(True)
    context.request_metadata["top_n_logprobs"].copy_(torch.tensor([0, 2, 1]))
    ...
    assert top_n_logprobs[1][0][0].tolist() == pytest.approx([-0.1, -0.2])
    assert top_n_logprobs[2][0][0].tolist() == pytest.approx([-1.0])


def test_async_sched_log_probs_materializes_accepted_mtp_tokens():
    context = _make_async_sched_context(total_request_count=2)
    context.request_metadata["return_log_probs"].fill_(True)
    context.request_metadata["top_n_logprobs"].copy_(torch.tensor([1, 2]))
    context.calculate_log_probs_tensors = mock.Mock(
        return_value=(
            torch.tensor([-0.1, -0.2, -9.0, -0.3, -0.4, -0.5]),
            torch.tensor(
                [
                    [-1.0, -2.0, -3.0, -4.0],
                    [-2.0, -1.0, -3.0, -4.0],
    ...
    controller = _make_async_sched_controller(context)
    controller.num_speculative_tokens = 2
    controller._all_logits_cuda = torch.empty(1, 6, 4)
    controller._accepted_tokens_per_request = torch.tensor([[10, -1], [11, 12]])
    controller._async_sched_selected_log_probs_cpu_buffer = torch.empty(6)
    sample_result = SimpleNamespace(
        sampled_tokens_gpu=torch.tensor([20, 21]), accepted_counts_gpu=torch.tensor([1, 2])
    )
    ...
    log_probs, top_n_logprobs = controller._materialize_async_sched_log_probs(
        transfer, torch.tensor([1, 2])
    )

    assert log_probs[0] == pytest.approx([-0.1, -0.2])
    assert log_probs[1] == pytest.approx([-0.3, -0.4, -0.5])
    assert len(top_n_logprobs[0]) == 2
    assert len(top_n_logprobs[1]) == 3
    assert torch.equal(top_n_logprobs[0][0][1], torch.tensor([0]))
    assert torch.equal(top_n_logprobs[1][0][1], torch.tensor([3, 2]))
    assert torch.equal(
        context.calculate_log_probs_tensors.call_args.args[1],
        torch.tensor([10, 20, 20, 11, 12, 21]),
    )
    assert torch.equal(
...
def test_async_sched_log_probs_materializes_mtp_mixed_rows():
    """Pack accepted decode rows and full prompt rows without losing boundaries."""
    context = _make_async_sched_context(total_request_count=3)
    context.config.materialize_only_last_token_logits = False
    ...
    context.request_metadata["return_log_probs"].fill_(True)
    context.request_metadata["top_n_logprobs"].copy_(torch.tensor([0, 0, 2]))
    context.request_metadata["skip_prompt_log_probs"].copy_(torch.tensor([False, False, True]))
    context.calculate_log_probs_tensors = mock.Mock(
        return_value=(
...
def test_run_async_sched_sample_records_gpu_ready_event():
    ...
    controller._copy_async_sched_sample_to_cpu = mock.Mock(
        return_value=(torch.empty(3, dtype=torch.int64), None, None, None)
    )
...
@pytest.mark.parametrize(
    "overlap, expected_waits",
    [(False, [mock.call("sample"), mock.call("repair")]), (True, [mock.call("accepted_counts")])],
)
def test_run_async_sched_mtp_rewind_orders_repairs_and_cpu_rewind(overlap, expected_waits):
    """MTP rewind waits only for state needed by the selected scheduling mode."""
    context = _make_async_sched_context(total_request_count=2)
    controller = _make_async_sched_controller(context)
    accepted_counts_gpu = torch.tensor([0, 1], dtype=torch.int64)
    accepted_counts_cpu = accepted_counts_gpu.clone()
    sample_result = SimpleNamespace(
        accepted_counts_gpu=accepted_counts_gpu,
        accepted_counts_cpu_view=accepted_counts_cpu,
        accepted_counts_cpu_ready_event="accepted_counts",
        sample_cpu_ready_event="sample",
    )
    blocks_to_reserve = torch.tensor([8, 9], dtype=torch.int32)
    reservation_mask = torch.tensor([True, False])
    controller._repair_async_sched_mamba_state = mock.Mock(return_value="repair")
    controller._synchronize_async_sched_event = mock.Mock()
    controller._rewind_kv_cache = mock.Mock(return_value=(blocks_to_reserve, reservation_mask))
...
def test_repair_async_sched_mamba_state_updates_live_state():
    """MTP repair writes the accepted state into the live Mamba buffer."""
    context = _make_async_sched_context(total_request_count=2)
    context.is_hybrid_model = True
    context.mamba_conv_states = torch.empty(1)
    controller = _make_async_sched_controller(context)
...
def test_run_async_sched_resolve_waits_only_for_finish_boundary(
    ...
    assert result.accepted_tokens_cpu is None
...
def test_run_async_sched_prefill_transition_resolves_before_prepare(drain_pending_forward, is_mtp):
    ...
    sample_result = SimpleNamespace(
        ...
        accepted_tokens_cpu_view=None,
        accepted_counts_gpu=torch.tensor([0, 1, 0]) if is_mtp else None,
        accepted_counts_cpu_view=torch.tensor([0, 1, 0]) if is_mtp else None,
        sample_cpu_ready_event="sample",
    )
    ...
    controller._run_async_sched_publish_bookkeeping = mock.Mock(return_value="bookkeeping")
    controller._run_async_sched_forward = mock.Mock(return_value="forward")

    result, cuda_graph_request_count, log_probs, top_n_logprobs = (
        controller._run_async_sched_prefill_transition(
    ...
    if drain_pending_forward:
        ...
        controller._run_async_sched_prepare.assert_called_once_with()
        assert torch.equal(
...
def test_run_async_sched_prefill_transition_stops_when_all_requests_finish():
    """Do not prepare a successor when prefill resolution has no survivors."""
    context = _make_async_sched_context(total_request_count=2)
    context.num_prefill_requests = 1
    controller = _make_async_sched_controller(context)
    ...
    controller._run_async_sched_log_probs = mock.Mock(return_value="log_probs_gpu")
    controller._copy_async_sched_log_probs_to_cpu = mock.Mock(
        return_value=SimpleNamespace(cpu_ready_event="log_probs")
    )
    controller._materialize_async_sched_log_probs = mock.Mock(return_value=([[0.1], [0.2]], None))
    controller._synchronize_async_sched_event = mock.Mock()
    controller._run_async_sched_resolve = mock.Mock(return_value=resolve_result)
    controller._run_async_sched_prepare = mock.Mock()

    result, _, log_probs, top_n_logprobs = controller._run_async_sched_prefill_transition(
        overlap=True, drain_pending_forward=False
    )
...
def test_async_sched_step_routes_prefill_transition():
    """Consume pending prefill logits through the transition phase."""
    context = _make_async_sched_context(total_request_count=2)
    context.num_prefill_requests = 1
    controller = _make_async_sched_controller(context)
    controller._run_async_sched_forward_primer = mock.Mock(return_value=(False, None))
    ...
    controller._run_async_sched_prefill_transition = mock.Mock(
        return_value=(resolve_result, 7, [[0.1], [0.2]], {0: [("prob", "token")]})
    )

    step_result = asyncio.run(controller._run_async_sched_step(overlap=True))

    assert step_result.output["sample"].tolist() == [1, 2]
    assert step_result.output["log_probs"] == [[0.1], [0.2]]
    assert step_result.output["top_n_logprobs"] == {0: [("prob", "token")]}
    assert step_result.output["cuda_graph_request_count"] == 7
    assert context.async_sched_step_count == 1
    controller._run_async_sched_prefill_transition.assert_called_once_with(
        overlap=True, drain_pending_forward=False
    )
...
def test_async_sched_mtp_step_uses_shared_tail(overlap, expected_call_order):
    """MTP adds verify/rewind before the common prepare, forward, and resolve flow."""
    context = _make_async_sched_context(total_request_count=2)
    context.active_token_count = 6
    controller = _make_async_sched_controller(context)
    controller.num_speculative_tokens = 2
    controller.num_mtp_depths = 2
    controller._async_sched_logits = AsyncScheduleLogitsState(
    ...
    controller._validate_async_sched_support_for_step = mock.Mock()
    call_order = []
    context.get_active_sequence_lengths = mock.Mock(
        side_effect=lambda: call_order.append("lengths") or torch.tensor([5, 7], dtype=torch.int32)
    )
    controller._run_async_sched_forward_primer = mock.Mock(return_value=(False, None))
    controller._synchronize_async_sched_event = mock.Mock(
        side_effect=lambda event: call_order.append(f"wait:{event}")
    )
    sampled_tokens = torch.tensor([20, 21], dtype=torch.int64)
    sampled_mtp_tokens = torch.tensor([[30, 31], [40, 41]], dtype=torch.int64)
    accepted_tokens = torch.tensor([[10, -1], [11, 12]], dtype=torch.int64)
    sample_result = SimpleNamespace(
        sampled_tokens_gpu=sampled_tokens,
        sampled_tokens_cpu_view=sampled_tokens.clone(),
        sampled_mtp_tokens_gpu=sampled_mtp_tokens,
        sampled_mtp_tokens_cpu_view=sampled_mtp_tokens.clone(),
        accepted_tokens_cpu_view=accepted_tokens,
        accepted_counts_gpu=torch.tensor([1, 2], dtype=torch.int64),
        accepted_counts_cpu_view=torch.tensor([1, 2], dtype=torch.int64),
        sample_cpu_ready_event="sample",
    )
    controller._run_async_sched_sample_mtp = mock.Mock(
        side_effect=lambda: call_order.append("sample_mtp") or sample_result
    ...
    controller._run_async_sched_sample = mock.Mock()
    controller._run_async_sched_mtp_rewind = mock.Mock(
        side_effect=lambda _result, *, overlap: call_order.append(f"rewind:{overlap}")
    )
    input_ids = torch.empty(1, 6, dtype=torch.int64)
    position_ids = torch.empty(1, 6, dtype=torch.int64)
    controller._run_async_sched_prepare = mock.Mock(
        side_effect=lambda: call_order.append("prepare") or (input_ids, position_ids)
    )
    context.copy_async_sched_sample_to_forward = mock.Mock(
    ...
    controller._materialize_async_sched_log_probs = mock.Mock(
        side_effect=lambda *_: call_order.append("materialize_log_probs") or (None, None)
    )
    controller._run_async_sched_publish_bookkeeping = mock.Mock(
        side_effect=lambda: call_order.append("publish") or "bookkeeping"
    )
    ...
    context.release_async_sched_rewound_kv_blocks = mock.Mock(
        side_effect=lambda: call_order.append("release_reservations")
    )
    controller._run_async_sched_resolve = mock.Mock(
        side_effect=lambda *_: call_order.append("resolve")
        or SimpleNamespace(
    ...
    controller._run_async_sched_sample.assert_not_called()
    assert torch.equal(result.output["accepted_tokens"], accepted_tokens)
    assert call_order == expected_call_order
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (AsyncScheduleMode.SERIAL, 1, False, "async"),
        (AsyncScheduleMode.SERIAL, 0, False, "async"),
        (AsyncScheduleMode.OVERLAP, 1, False, "overlap"),
...
def test_async_generate_output_tokens_dynamic_batch_routes(
    ...
    if expected_result != "legacy":
        controller._run_async_sched_step.assert_awaited_once_with(
            overlap=mode == AsyncScheduleMode.OVERLAP, drain_pending_forward=False
        )
...
class TestTextGenerationController(TextGenerationControllerTestBase):
    ...
    def test_rewind_kv_cache(self, is_hybrid_model):
        ...
        self.text_generation_controller._rewind_mamba_state(
            self.text_generation_controller._accepted_token_counts_per_request[:2]
        )
